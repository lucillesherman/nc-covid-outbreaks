...
### FIFTH DHHS REPORT
# cleaned facilities in text editor before importing
...
# combine
alloutbreaks <- bind_rows(april27outbreaks, may1outbreaks, may5outbreaks, may8outbreaks, may12outbreaks)
...
may12outbreaks %>% filter(facility_type == "Nursing Home") %>% 
  adorn_totals(where = "row") %>% tail(5)
...
alloutbreaks %>% 
  ...
  mutate(diff = `5/12` - `5/8`) %>% View()
mutate(cases512 = as.numeric(ifelse(releasedate == "5/12", total_cases, NA))) %>% 
  mutate(cases58 = as.numeric(ifelse(releasedate == "5/8", total_cases, NA))) %>% 
  filter(releasedate == "5/12" | releasedate == "5/8") %>% 
...
lasttwo <- alloutbreaks %>% 
  filter(releasedate == "5/8" | releasedate == "5/12") %>% 
  count(facility)
